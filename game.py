...
def get_player_name():
    ...
    while not done:
        screen.fill((0, 0, 0))  # Dark background for better visibility

        # Draw a semi-transparent background for the input box area
        pygame.draw.rect(screen, (0, 0, 0, 180), pygame.Rect(0, 0, screen_width, screen_height))

        # Render the "Enter your name" label
        label_font = pygame.font.SysFont('Bauhaus 93', 30)
...
while run:
    ...
    if game_over == True:
        if not score_submitted:
            submit_score(score, player_name)  # Submit with player name
